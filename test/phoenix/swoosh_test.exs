defmodule Phoenix.SwooshTest do
  ...
  defmodule TestEmail do
    ...
    def welcome_html(), do: email() |> template_body("welcome.html", %{})
    def welcome_text(), do: email() |> template_body("welcome.text", %{})

    def welcome_html_assigns(), do: email() |> template_body("welcome_assigns.html", %{name: "Tony"})
    def welcome_text_assigns(), do: email() |> template_body("welcome_assigns.text", %{name: "Tony"})

    def welcome_html_layout() do
      email()
      |> put_layout({LayoutView, "email.html"})
      |> template_body("welcome.html", %{})
    end

    def welcome_text_layout() do
      email()
      |> put_layout({LayoutView, "email.text"})
      |> template_body("welcome.text", %{})
    end

    def welcome_html_layout_assigns() do
      email()
      |> put_layout({LayoutView, "email.html"})
      |> template_body("welcome_assigns.html", %{name: "Tony"})
    end

    def welcome_text_layout_assigns() do
      email()
      |> put_layout({LayoutView, "email.text"})
      |> template_body("welcome_assigns.text", %{name: "Tony"})
    end

    def welcome(), do: email() |> template_body(:welcome, %{})

    def welcome_assigns(), do: email() |> template_body(:welcome_assigns, %{name: "Tony"})

    def welcome_layout() do
      email()
      |> put_layout({LayoutView, :email})
      |> template_body(:welcome, %{})
    end

    def welcome_layout_assigns() do
      email()
      |> put_layout({LayoutView, :email})
      |> template_body(:welcome_assigns, %{name: "Tony"})
    end
